"""Model definitions for MNIST-style image experiments."""
...
def get_model(model_name: str = "small_cnn", dataset: str = "mnist") -> nn.Module:
    ...
    if normalized_dataset not in {"mnist", "fashion_mnist"}:
        raise ValueError(
            f"Unsupported dataset '{dataset}'. Expected 'mnist' or 'fashion_mnist'."
        )
